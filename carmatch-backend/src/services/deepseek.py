...
CAR_RESPONSE_SEPARATOR = "_____________________________"
...
PROMPT_EXTRACT_PARAMS = """Ты — система извлечения параметров подбора автомобиля. Язык диалога — русский.

ИСТОЧНИК ПАРАМЕТРОВ — ТОЛЬКО СООБЩЕНИЯ ПОЛЬЗОВАТЕЛЯ (роль "user"). Сообщения ассистента (роль "assistant") — только для контекста (понять, на какой вопрос отвечает пользователь). Из текста ассистента НИ ОДИН параметр не извлекай. Значения brand, model, body_type, year и т.д. бери исключительно из того, что написал сам пользователь.

ЗАПРЕЩЕНО ВЫДУМЫВАТЬ: не додумывай, не предполагай и не подставляй параметры, которых пользователь не написал. Если пользователь не упоминал марку — не добавляй brand. Не упоминал модель — не добавляй model. Не говорил про год, топливо, коробку, кузов — не добавляй их. В JSON попадают только параметры, которые явно есть в тексте сообщений пользователя (или уже есть в «Текущие уже собранные»). Пустой массив или только уже собранные параметры — нормально, если пользователь ничего нового не написал.

Твоя задача: пройти по КАЖДОМУ сообщению пользователя (user) по очереди и извлечь из него только явно упомянутые параметры. Ответы ассистента не анализируй на предмет параметров.

Допустимые параметры (для поиска нужно минимум три любых):
- brand — марка (Toyota, BMW, Lada и т.д.). Только если пользователь назвал марку.
- model — модель (Camry, X5, Vesta). Только если пользователь назвал модель.
- body_type — тип кузова. СТРОГО из списка ниже (ничего другого не подставляй). Только если пользователь назвал кузов (седан, хэтчбек и т.д.):

BODY_TYPE_REFERENCE_PLACEHOLDER

- year — год выпуска. value — строка с числом ("2020"). Только если пользователь указал год.
- modification — полная строка модификации. Только если пользователь написал.
- fuel_type — тип топлива: ровно одно из бензин, дизель, гибрид, электро. Только если пользователь сказал про топливо («бензин», «на дизеле» и т.п.). Не путай с коробкой.
- transmission — коробка (автомат, механика, вариатор, робот). Только если пользователь сказал про коробку. Не путай с топливом.
- engine_volume — объём двигателя (1.4, 1.6). Только если пользователь указал.
- horsepower — мощность в л.с. Только если пользователь указал.

Текущие уже собранные параметры сессии: CURRENT_PARAMS_PLACEHOLDER

Логика: итоговый список = уже собранные параметры + новые, извлечённые только из сообщений пользователя. По каждому сообщению user смотри: что он явно написал (марка, модель, кузов, год, топливо, коробка и т.д.) — только это и добавляй. Если в сообщении пользователя по какому-то параметру ничего нет — оставляй значение из «Текущие уже собранные» или не включай параметр. Сообщения assistant игнорируй как источник значений.

ПРАВИЛО ПЕРЕЗАПИСИ: если пользователь в более позднем сообщении изменил выбор (сначала «рено», потом «бмв»), в JSON только последнее значение по каждому типу (brand: BMW).

Формат вывода (строго): в конце ответа ровно один блок JSON:
```json
{"extracted_params": [{"type": "brand", "value": "Toyota", "confidence": 0.95}, {"type": "model", "value": "Camry", "confidence": 0.9}]}
```
Включай в массив только те параметры, которые пользователь реально упомянул в своих сообщениях (или уже есть в текущих собранных). value — строка; confidence — число от 0 до 1. body_type — строго из списка выше. Не выдумывай type/value."""
...
def extract_params(
    ...
    if last_user_content:
        system_content += (
            "\n\n--- Последнее сообщение пользователя (источник — только из него и других user-сообщений извлекай параметры, ничего не додумывай): «"
            + last_user_content[:500]
            + ("»" if len(last_user_content) <= 500 else "» (обрезано). ")
            + " Включи в extracted_params только то, что пользователь здесь или в предыдущих своих сообщениях явно написал."
        )
    api_messages = [{"role": "system", "content": system_content}]
    for m in messages:
        role = m.get("role", "user")
    ...
    try:
        ...
        return []
    if not raw:
        logger.warning("extract_params: LLM вернул пустой ответ")
        return []
    parsed = _parse_extract_params_response(raw)
    llm_params_str = ", ".join(f"{p.get('type')}={p.get('value')}" for p in parsed)
    logger.info(
        "extract_params: raw_len=%s, parsed_count=%s, LLM извлёк: %s",
        len(raw or ""),
        len(parsed),
        llm_params_str or "(ничего)",
    )
    if not parsed:
        # Чтобы в логах было видно, что именно вернул LLM (и почему парсинг дал 0 параметров)
        raw_preview = (raw or "").strip()[:1200]
        logger.warning(
            "extract_params: LLM не вернул валидный JSON с extracted_params (parsed=0). "
            "Начало ответа LLM (для отладки): %s",
            raw_preview + ("..." if len((raw or "").strip()) > 1200 else ""),
        )
    return parsed
...
def _format_cars_for_user_answer(search_results: list) -> str:
    ...
    for car in cars_to_show:
        ...
        lines.append(CAR_RESPONSE_SEPARATOR)
        lines.append("")

    return "\n".join(lines)


def _ensure_separators_between_cars(text: str) -> str:
    """
    Делает разделители между блоками машин единообразными: после каждой строки с ℹ️
    перед следующим блоком 🚗 должна быть ровно одна линия подчёркиваний. Если LLM
    её пропустил — вставляем.
    """
    if not text or "🚗" not in text:
        return text
    lines = text.split("\n")
    result: list[str] = []
    need_sep_before_next_car = False
    for line in lines:
        stripped = line.strip()
        if stripped.startswith("🚗"):
            if need_sep_before_next_car:
                result.append(CAR_RESPONSE_SEPARATOR)
            result.append(line)
            need_sep_before_next_car = False
        elif stripped.startswith("ℹ️"):
            result.append(line)
            need_sep_before_next_car = True
        elif re.match(r"^_{5,}\s*$", stripped):
            result.append(line)
            need_sep_before_next_car = False
        else:
            result.append(line)
...
def _normalize_car_response_icons(text: str) -> str:
    ...
    убирает лишнее «Страна производства». Затем нормализует разделители между блоками машин.
    """
    ...
    t = re.sub(r"^\s*разделитель\s*$", CAR_RESPONSE_SEPARATOR, t, flags=re.MULTILINE)
    ...
    t = _ensure_separators_between_cars(t)
    return t
