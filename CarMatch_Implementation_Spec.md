...
- **B15.** Реализовать скрипт или команду импорта каталога из Yandex Auto Export XML в таблицу cars (парсинг XML, маппинг полей, batch insert). URL XML: [https://auto-export.s3.yandex.net/auto/price-list/catalog/cars.xml](https://auto-export.s3.yandex.net/auto/price-list/catalog/cars.xml) (или локальный файл cars.xml).
...
- **F7.** Создать `src/components/ProtectedRoute.tsx`: читать token только из AuthContext (useContext(AuthContext).token); при отсутствии token — , иначе —  или children.
...
- **F16.** Добавить в .env.example переменную VITE_API_BASE_URL=[http://localhost:8000](http://localhost:8000) (или URL бэкенда).
